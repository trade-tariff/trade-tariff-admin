module ApiEntity
  ...
  class CollectionProxy < SimpleDelegator
    ...
    def find(id)
      result = @collection.find { |entry| entry.to_param == id }
  ...
  included do
    ...
    attr_accessor :resource_id, :casted_by
  ...
  def save
    resp = begin
      if persisted?
        api.put(singular_path, serializable_hash)
      else
...
private
  ...
  module ClassMethods
    ...
    def collection(opts = {})
      ...
      meta = resp.body.fetch('meta', {})
      meta = JSON.parse(resp.headers.fetch('x-meta', '{}')).merge(meta)
    ...
    def parse_jsonapi(resp)
      TariffJsonapiParser.new(
        resp.try(:body) || resp.try(:[], :body),
      ).parse
