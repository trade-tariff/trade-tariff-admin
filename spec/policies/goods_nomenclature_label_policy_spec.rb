RSpec.describe GoodsNomenclatureLabelPolicy do
  subject(:policy) { described_class }

  let(:label) { GoodsNomenclatureLabel.new(goods_nomenclature_sid: 12_345) }

  permissions :index?, :show?, :update? do
    it "grants access to technical operator" do
      user = create(:user, :technical_operator)
      expect(policy).to permit(user, label)
    end

    it "denies access to hmrc admin" do
      user = create(:user, :hmrc_admin)
      expect(policy).not_to permit(user, label)
    end
