RSpec.describe "Search analytics dashboard" do
  ...
  def expect_default_dashboard_content
    ...
    expect(page).to have_content("Searches by request source")
    expect(page).to have_content("Frontend-routed")
    expect(page).to have_content("Direct backend / non-frontend")
    expect(page).to have_content("Unknown")
    expect(page).to have_content("915")
