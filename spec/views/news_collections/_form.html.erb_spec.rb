  ...
  context 'with valid news collection' do
    ...
    it 'populates fields' do
      expect(form).to have_css \
        %(input[type="text"][name$="[name]"][value="#{news_collection.name}"])

      expect(form).to have_css \
        %(input[type="text"][name$="[priority]"][value="#{news_collection.priority}"])

      expect(form).to have_css \
        %([id$="news-collection-published-true-field"][checked="checked"])
    end
